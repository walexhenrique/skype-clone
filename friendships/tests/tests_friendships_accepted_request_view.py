...
class AcceptedRequestViewTest(FriendshipsBaseTest):
    def setUp(self) -> None:
        self.url = reverse('friendships:profile-accepted', kwargs={'id': 1})
        return super().setUp()
    
    def test_accepted_request_view_redirect_for_login_if_is_not_logged(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 302)
    
    def test_accepted_request_the_profile_owner_accept_the_request_and_modifies_the_status_correctly(self):
        ...
        friend_request_accepted = Friend.objects.filter(
            pk=1,
        ...
        self.assertTrue(friend_request_accepted)

    def test_accepted_request_the_profile_the_owner_tries_to_accept_a_request_that_does_not_exist_or_does_not_belong_to_him(self):
        # you must be logged in
        self.client.login(username='Breno', password='123456')
        ...
        self.assertFalse(friend_request_accepted)
